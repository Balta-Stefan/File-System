...
namespace CustomFS
{
    ...
    class Program
    {
        ...
        static void runProgram()
        {
            ...
            try
            {
                obj.filesystem = new KIRZFilesystem(obj.encryptionKey, obj.hashingAlgorithm, obj.encryptionAlgorithm, obj.keyPair, new Filesystem.LoginCredentials(obj.username, obj.password, obj.clientCertificate));
            }
            catch (Exception) { Console.WriteLine("Login unsuccessful.");return; }
            ...
            while (true)
            {
                ...
                if (command.Equals("exit"))
                {
                    ...
                    Array.Clear(obj.encryptionKey, 0, obj.encryptionKey.Length);
                    return;
                }
